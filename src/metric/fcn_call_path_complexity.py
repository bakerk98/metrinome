...
class FunctionCallPathComplexity(ABC):
    ...
    def evaluate(self, cfg: ControlFlowGraph, all_cfgs, branching : str = "branching") -> Union[int, PathComplexityRes]:
        ...
        # TODO: delete for real testing
        self.apc_times['gamma'] = optimizedGamma
        ...
        self.apc_times['firstHalfTime'] = gammaTime + \
            graphProcessTime + graphSystemsTime
    ...
    def fcn_call_apc(self, gamma):
        ...
        if numroots == 0:
            ...
            if len(possibleGenFunc) == 1:
                ...
                # simplifies generating function :)
                genFunc = sympy.simplify(sympy.factor(genFunc))
                self.logger.d_msg(f"Generating Function: {genFunc}")
            else:
                self.logger.e_msg(
                    "PANIC PANIC Oh dear, not sure which generating function is right")
            ...
            if numRoots < maxPow:  # cannot solve symbolically for roots, approximating solutions via nroots
                self.logger.d_msg(
                    "numRoots < maxPow, approximating roots via nroots")
                newRootsDict = {}
                approxroots = sympy.nroots(
                    denominator, n=(PRECISION + 1), maxsteps=1000)
        ...
        # case 2: discriminant has real roots
            ...
            start_time = time.time()
            rStar = min(map(lambda x: x if x > 10**(-PRECISION)
                        else sympy.oo, self.realnroots(discrim)))
    ...
    def processGraphs(self, graph, all_graphs):
        ...
        # list of graphs left to process for current function (graphs of functions that are called + graph of original function)
        ...
        while graphs_to_process:
            ...
            # index graphs for use in systems later
            fcn_idx = used_graphs.index(curr_graph_name.split(".")[-1])
            curr_graph_edges = [(f'{fcn_idx}_{edge[0]}', f'{fcn_idx}_{edge[1]}')
                                for edge in curr_graph_edges]  # rename vertices for use in systems later
            ...
            for node in curr_graph_calls.keys():  # loop through nodes that call other functions
                ...
                for called_fcn in curr_graph_calls[node].split(" "):
                    ...
                    if called_fcn not in used_graphs:  # if function hasn't been called and processed earlier
                        ...
                        for graph in all_graphs:
                            if graph.split(".")[-2] == called_fcn:
                                # add new functions to list of graphs to process
                    ...
                    calldict[f'{fcn_idx}_{int(node)}'].append(
    ...
    def graphsToSystems(self, dictgraphs, calldict):
        ...
        lookupDict = defaultdict(
            set)  # lookupDict is of format {symb: [every symbol whose equation contains symb]}, used for substitution in eliminate
        ...
        # (list of lists) list of graphs' systems (each system is a list of symbolic equations)
        splitsystems = []
        splitsymbols = []  # (list of lists) list of graphs' symbols
        init_eqns = []  # list of initial equations

        # loop through all function graphs in order
        for fcn_idx in range(len(dictgraphs)):
            ...
            for startnode in curr_graph:  # each non-terminal node/symbol has its own equation, loop builds the expression equal to each non-terminal node
                ...
                # adding the symbol to the list of symbols in this symbol's equation
                lookupDict[sym].add(sym)
                # adding the index of this symbol's eqn into the idxDict
                ...
                for node in endnodes:  # constructing the equation for this node/symbol by looping through each child of the node/symbol
                    ...
                    # adding our parent node/symbol to the lookupDict for each child of the node/symbol
                    ...
                    if str(node) in curr_graph:  # makes sure the child node is not terminal
                        ...
                        add_to_expr = x
                        # terminal child node calls function
                        for called_fcn_idx in calldict[node]:
                            # include terminal child's function calls
                            add_to_expr = init_nodes[called_fcn_idx] * \
                                add_to_expr
                        expr = expr + add_to_expr  # adding terminal nodes/children to the expression

                # handle function calls for all non-terminal nodes  (all nodes in startnodes)
    ...
    def optimizedEliminate(self, systems, symbs, lookupDict, idxDict):
        ...
        for idx, system in enumerate(systems):
            # solving system with helper function
            Teqn = self.optimizedPartialEliminate(
                system, symbs[idx], lookupDict, True, idxDict)
        ...
        gamma = self.optimizedPartialEliminate(
            Teqns, Tsyms, TlookupDict, False, idxDict)
    ...
    def optimizedPartialEliminate(self, system, symbs, lookupDict, vertices: bool, idxDict):
        ...
        if s in sub.free_symbols:
            ...
            if vertices == False:
                subs_options = self.simpleOrder(
                    system, subs_options, idxDict, vertices)
        ...
        if s in sub.free_symbols:
            self.logger.e_msg(
                f"PANIC PANIC no unique solution found for {s}, not sure how to substitute.")
            ...
            if eq_idx <= symb_idx:
                ...
                if s in eq.free_symbols:
                    ...
                    if s in system[eq_idx].free_symbols:
                        self.logger.e_msg(
                            f"PANIC PANIC failed to substitute solution for {s} not sure how to substitute.")
    ...
    def simpleOrder(self, system, symblist, idxDict, vertices):
        ...
        sorted_symbols_by_simplicity = sorted(
            lengthDictionary.items(), key=lambda x: x[1], reverse=False)
    ...
    def getAPC(self, genFunc, denominator, rootsDict):
        ...
        for rho in rhoDict:
            ...
            if (abs(abs(rho) - maxRho) < 10**(-10)) and (rhoDict[rho] == maxMultiplicity):
                Ak = sympy.N(self.shiftAk(numerator, rho) /
                             self.calculateAk(q0, rho, rhoDict))
                self.logger.d_msg(f"Ak for {rho}: {Ak}")
                coeff = coeff + Ak

        self.logger.d_msg(f"coeff: {coeff}")
        apc = sympy.simplify(
            coeff*(symbols("n")**(maxMultiplicity-1))*sympy.N(maxRho)**symbols("n"))
        return apc
    ...
    def getRhoDict(self, rootsDict):
        ...
        for root in rootsDict:
            ...
            # if the difference in the absolute value is greater than 10 decimal
            if (abs(rho) - abs(maxRho) >= 10**(-10)):
                maxRho = rho
                maxMultiplicity = multiplicity
            # abs(rho) == abs(maxRho) within 10 decimal
    ...
    def realnroots(self, eq):
        ...
        if "/" in str(eq):
            ...
            numroots = sympy.nroots(
                numerator, n=(PRECISION + 1), maxsteps=1000)
            self.logger.d_msg(f"numroots: {numroots}")
            denoroots = sympy.nroots(
                denominator, n=(PRECISION + 1), maxsteps=1000)
